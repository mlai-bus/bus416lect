...
There are two packages that we will use to import data. `readr` package is used to import CSV files and `readxl` is used to read Excel files. Though the preference is to use the `.csv` format.

Also, note that the data file is stored in a separate folder, `/data` within the project folder. This is good practice as it separates out code from data. Some large projects may involve setting up two folders: `/data_raw` and `/data_clean`.
...
```{r}
# Pressing tab inside quotes will list available files
...
**Note:** The above method of reading CSVs using `read_csv()` works well when you have external data files in the project directory.

However, in this course, most datasets are contained in the `mlbadata` library and can be loaded directly.
...
There are two common ways to access a variable: using the `$` accessor or using column references. Each results in a different class of object, which has implications for what operations are possible.
...
### Accessing columns in different ways
...
The result is a vector object, which allows direct mathematical operations.
...
### Column references return data frames / tibbles
...
`REMODEL` has three values: "Recent", "None", and "Old". We want to make this into a factor and control the order of the levels.
...
`R` provides several ways to create frequency tables and pivot-style summaries.
...
### Student practice: two-way table
...
### Student practice: overall proportions
...
### Student practice: row-wise proportions
...
### Student practice: column-wise proportions
