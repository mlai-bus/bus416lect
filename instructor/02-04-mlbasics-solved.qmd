...
We will create an 80–20 split using **caret**.
...
---
###Student exercises (solved): ML workflow practice
...
test_predictions <- predict(reg_final, newdata = test_df)
...
```{r}
train_results <- augment(reg_final) |> 
  select(TOTAL.VALUE, .fitted, .resid)
