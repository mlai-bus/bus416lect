...
Let us use this model to predict car prices in our validation set.

```{r}

head(augment(lm.1, newdata = valid.df))
# as we saw before, augmented values are prepended by.

valid.pred <- augment(lm.1, newdata = valid.df) |> 
  rename(Pricehat = .fitted,
         resid = .resid)

# check rmse etc
accuracy(valid.pred$Pricehat, valid.df$Price)
