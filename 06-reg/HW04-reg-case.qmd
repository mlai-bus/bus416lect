...
In this case study we will run various regression model selection algorithms to predict the price of used Corollas over the entire dataset and over the set of 39 feature variables.

### Your Assignment:

Below I have provided the code for carrying out a forward selection algorithm in `caret`. Go through the code carefully and replicate the analysis for either backward or sequential selection (that is, either part 2 or 3). Then, ideally, complete parts 4-6 but for sure part 4 (Lasso regression). Then compare the performance of all the models on the test set.
