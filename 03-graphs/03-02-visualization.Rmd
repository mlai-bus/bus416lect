...
utilities.df <- read_csv("Utilities.csv")
ebay.df <- read_csv("EbayTreemap.csv")
```
...
One can obtain a density plot, separrated by CHAS:
...
```{r}
gg3 <- housing.df |> 
  ggplot(aes(x = MEDV,y = ..density..)) +
  geom_histogram(aes( fill = factor(CHAS)), alpha = 0.25) +
  geom_density(aes( color = factor(CHAS)), size = 2) +
...
To use ggplot funtion `geom_tile()`, which let's one put in values, we first need to reshape the corrletion matrix to a long (tidy) format.
...
gg6 <- plot.data |> 
  ggplot(aes(x = RAD, y = avg_MEDV)) +
  geom_bar(stat = "identity") +
  facet_wrap(vars(CHAS), nrow = 2) +
